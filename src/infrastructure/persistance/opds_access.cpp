...
void OpdsAccess::loadRootLib(const QString& url)
{
        ...
        [this, reply]()
        {
            ...
            if(!success)
            {
                api_error_helper::logErrorMessage(reply,
                                                  "Fetching opds feed  page");
                emit badNetworkResponse(
    ...
    connect(reply, &QNetworkReply::redirected, this,
            [this, reply](const QUrl _url)
            {
                this->redirected(reply, _url);
            });
}
...
void OpdsAccess::getBookMedia(const QString& id, const QUuid& uuid,
                              const QString& _url)
{
    ...
    connect(reply, &QNetworkReply::redirected, this,
            [this, reply](const QUrl _rurl)
            {
                this->redirected(reply, _rurl);
            });
    ...
    connect(
        ...
        [this, reply, id, uuid, format]()
        {
            ...
            emit gettingBookMediaChunkReady(id, uuid, QByteArray(),
                                            changeFormat(reply, format), true);
            reply->deleteLater();
        });
...
QString OpdsAccess::changeFormat(const QNetworkReply* const reply,
