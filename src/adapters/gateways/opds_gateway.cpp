...
{
    connect(m_OpdsAccess, &IOpdsAccess::loadOpdsRootFinished, this,
            &OpdsGateway::parseOpdsResonse);

    connect(m_OpdsAccess, &IOpdsAccess::gettingOpdsImageFinished, this,
            &OpdsGateway::processOpdsImage);

    connect(m_OpdsAccess, &IOpdsAccess::badNetworkResponse, this,
            &IOpdsGateway::badNetworkResponse);
    // send to service
    connect(m_OpdsAccess, &IOpdsAccess::gettingBookMediaChunkReady, this,
            &IOpdsGateway::gettingBookMediaChunkReady);

    connect(m_OpdsAccess, &IOpdsAccess::gettingBookMediaProgressChanged, this,
...
void OpdsGateway::convertRelativeUrlToAbsolute(QString& url)
{
    ...
    if(!new_url.isRelative())
        return;
    if(!new_url.path().startsWith("/"))
        new_url.setPath("/" + new_url.path());
...
void OpdsGateway::parseOpdsResonse(const QByteArray& data)
{
    ...
    for(auto it = parser.dom.entries.cbegin(); it != parser.dom.entries.end();
        ++it)
    {
        if(it->title.empty())
            continue;  // skip entries with empty title
        // accumulate authors
        QString authors;
        for(auto it_author = it->authors.cbegin();
            it_author != it->authors.cend(); ++it_author)
        {
        ...
        std::vector<std::pair<std::string, std::string>> link_vec =
            parser.getDownloadUrlsByID(it->id);
        for(auto it_dLink = link_vec.cbegin(); it_dLink != link_vec.cend();
            ++it_dLink)
        {
            downloadUrls.append(
                QPair(QString(it_dLink->first.c_str()),
        ...
        {
            QTextDocument doc;
            for(auto it_content = it->content.cbegin();
                it_content != it->content.cend(); ++it_content)
            {
    ...
    auto nextLinkIterator = std::find_if(
        parser.dom.links.cbegin(), parser.dom.links.cend(),
        [](const Link& link)
        {
            return link.rel == "next" &&
                   link.type.find("atom+xml") != std::string::npos &&
                   !link.href.empty();
        });
    if(nextLinkIterator != parser.dom.links.cend())
    {
        res.emplace_back("Next", "", nextLinkIterator->href.c_str(), "Next",
                         "id_next", "");
    }
    ...
    //        auto it2 = std::find_if(it,res.end(), [&it](const OpdsNode&
    //        node1){
    //            if (it->title == node1.title && it->author == it->author &&
    //            it->imageUrl == node1.imageUrl && it->id!=node1.id){
    //                    return true;
    //            }
    //            else return false;
    //        });
...
void OpdsGateway::getBookMedia(const QString& id, const QUuid& uuid,
                               const QString& url)
{
