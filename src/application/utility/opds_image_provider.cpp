...
OpdsImageProvider::OpdsImageProvider(IOpdsService* opdsService)
    : QQuickImageProvider(QQmlImageProviderBase::Image),
      ptrService(opdsService)
{
}

QImage OpdsImageProvider::requestImage(const QString& url, QSize* size,
                                       const QSize& requestedSize)
{
    ...
    if(!result.isNull() && requestedSize.height() > 80 &&
       requestedSize.width() > 50 &&
